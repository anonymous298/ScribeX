...
import { deleteNote, fetchAllStarredNotes, getAllNotes, toggleStarredNote, updateNote } from "@/server/actions/note.action";
...
import { NoteCardSkeleton } from "../dashboard/NoteCardSkeleton";
import CreateNoteButtonForMainPage from "../dashboard/CreateNoteButtonForMainPage";
...
export function AllStarredNotesComponent() {
    ...
    useEffect(() => {
        const fetchNotes = async () => {
            ...
            const allStarredNotes = await fetchAllStarredNotes();

            setStarredNotes(allStarredNotes ?? []);

            setIsLoadingNotes(false)

            if (allStarredNotes?.length === 0) {
    ...
    if (isNotesNotPresent) return <p>No Starred Notes Yet</p>
...
export default AllStarredNotesComponent
